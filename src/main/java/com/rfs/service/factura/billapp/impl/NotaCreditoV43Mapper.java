...
public class NotaCreditoV43Mapper implements Mapper {
    ...
    public Object mapFacturaElectronica(Object o, String tipoDocumento) throws DatatypeConfigurationException {
        ...
        fe.setDetalleServicio(createDetalleServicio(data, issueDate));
    ...
    private NotaCreditoElectronica.DetalleServicio createDetalleServicio(NotaCreditoDataDTO data, XMLGregorianCalendar date) {
        NotaCreditoElectronica.DetalleServicio detalleServicio = new NotaCreditoElectronica.DetalleServicio();
        createDetalle(data.getDetallesDTO(),detalleServicio, data.getClienteObject(), date);
        return detalleServicio;

    }

    private void createDetalle(List<NotaCreditoDetalleDTO> impuestos,
                               NotaCreditoElectronica.DetalleServicio detalleServicio,
...
//        return i.doubleValue();
        ...
        if (fd.getTarifaIva()!=null && fd.getTarifaIva().getCodigo()!=null) {
            it.setCodigoTarifa(fd.getTarifaIva().getCodigo()); //13%
            it.setTarifa(createDinero(new Double(fd.getTarifaIva().getPorcentaje())));

        } else {
            it.setCodigoTarifa("01");
            it.setTarifa(createDinero(new Double(0)));
        }
        it.setMonto(i);
        ...
        if (!estaClienteExonerado(c) && fd.getMedida()!=null && BillHelper.UNIT_PRODUCTO.equalsIgnoreCase(fd.getMedida().getCategoria())) {
            ...
            this.totalServiciosGrabados = this.totalServiciosGrabados + fd.getMontoNeto();
        } else if (!estaClienteExonerado(c) && ld.getUnidadMedida().equals("Sp")) {
            this.totalServiciosGrabados = this.totalServiciosGrabados + fd.getMontoNeto();
        }
//        else {
//            this.totalServiciosGrabados = this.totalServiciosGrabados + fd.getMontoNeto();
...
//        return resumenFactura;
        ...
        setCodigoMonedaIfNeedIt(data, resumenFactura);
        resumenFactura.setTotalServGravados(createDinero(this.totalServiciosGrabados));
        resumenFactura.setTotalServExentos(createDinero(this.totalServiciosExcento));
    ...
    private void setCodigoMonedaIfNeedIt(NotaCreditoDataDTO invoice, NotaCreditoElectronica.ResumenFactura resumenFactura) {
